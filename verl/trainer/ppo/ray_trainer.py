...
def log_progress_metrics(data: DataProto):
    progress_metrics = {}
    # print(data.non_tensor_batch.keys())
    for key in data.non_tensor_batch.keys():
        if 'log_metric' in key:
            progress_metrics[key] = np.mean(data.non_tensor_batch[key].astype(np.float32)).item()
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, step_advantage_w=1.0, gigpo_mode="mean_std_norm", gigpo_enable_similarity=False, gigpo_similarity_thresh=0.95, **kwargs):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns, mean_group_std = core_algos.compute_grpo_outcome_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
        # data.batch["mean_group_std"] = mean_group_std
        extras["mean_group_std"] = mean_group_std
        ...
        def get_stage(raw_prompt):
            if 'At each conversation, you are required to use exactly one of the following actions:' in raw_prompt:
                return 'retrieve'
            elif 'which shows the scene from view' in raw_prompt:
                return 'update_cogmap'
            else:
                return 'answer'

        def get_stage_rewards(data_item):
            record = {}
            for k, v in data_item.non_tensor_batch.items():
                if k.startswith('progress_'): # or k.startswith('log_metric'):
                    record[k] = v
            return record

        stage = list(map(lambda x: get_stage(x[0]['content']), data.non_tensor_batch['raw_prompt']))
        stage_rewards = list(map(lambda x: get_stage_rewards(x), data))

        advantages, returns, mean_group_std = core_algos.compute_grpo_progress_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
            index=data.non_tensor_batch["uid"],
            traj_index=data.non_tensor_batch['traj_uid'],
...
class RayPPOTrainer:
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler):
        ...
        assert len(self.train_dataset) >= 1, "Train dataloader is empty!"
        assert len(self.val_dataset) >= 1, "Validation dataloader is empty!"

        print(f"Size of train dataloader: {len(self.train_dataset)}, Size of val dataloader: {len(self.val_dataset)}")
    ...
    def _dump_generations(self, traj_indices, inputs, outputs, scores, reward_extra_infos_dict, dump_path):
    ...
    def inference(self):
        ...
        for test_data in self.val_dataloader:
            test_batch = DataProto.from_single_dict(test_data)
            pprint(test_batch.non_tensor_batch['index'])
            timer_raw = {}
            ...
            ################ agent-environment loop ###############
            with _timer("multi_turn_generation", timer_raw):
                test_output_gen_batch = self.traj_collector.multi_turn_generation(
                                                        gen_batch=test_gen_batch,
                                                        actor_rollout_wg=self.actor_rollout_wg,
                                                        envs=self.val_envs,
                                                        index=test_data['index'],
                                                        )
            print(f"multi_turn_generation time: {timer_raw['multi_turn_generation']}")
            output_list.extend(test_output_gen_batch)
            del test_batch
    ...
    def fit(self):
        ...
        with Progress() as progress:
            progress_task = progress.add_task("Training", total=self.total_training_steps)
            for epoch in range(self.config.trainer.total_epochs):
                for batch_dict in self.train_dataloader:
                    print(batch_dict['index'])
                    metrics = {}
                    timing_raw = {}
                    ...
                    with _timer("step", timing_raw):
                        ...
                        progress_metrics = log_progress_metrics(batch)
                        ...
                        with _timer("adv", timing_raw):
                            ...
                            #     batch.non_tensor_batch.update({k: v for k, v in reward_extra_infos_dict.items()})
                            ...
                            batch, extras = compute_advantage(
                                batch,
                        ...
                        if rollout_data_dir:
                            with _timer("dump_rollout_generations", timing_raw):
                                ...
                                self._dump_generations(
                                    traj_indices=batch.non_tensor_batch['traj_uid'],
                                    inputs=inputs,
                                    outputs=outputs,
                                    scores=scores,
                    ...
                    # if not os.path.exists('test_batch.pkl'):
