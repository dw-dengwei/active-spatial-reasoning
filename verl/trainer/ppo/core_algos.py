...
def compute_grpo_outcome_advantage(
    ...
    mean_group_std = torch.mean(torch.stack(list(id2std.values()))).item()

    return scores, scores, mean_group_std


def compute_grpo_progress_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    traj_index: np.ndarray,
    stage: list[str],
    stage_rewards: list[dict],
    epsilon: float = 1e-6,
    norm_adv_by_std_in_grpo: str = True,
    compute_mean_std_cross_steps: bool = True,
):
    ...
    def get_stage_reward(stage_name, stage_rewards_dict):
        if stage_name == 'retrieve':
            ret = stage_rewards_dict['progress_retrieve']
        elif stage_name == 'update_cogmap':
            ret = stage_rewards_dict['progress_cogmap']
        elif stage_name == 'answer':
            ret = stage_rewards_dict['progress_answer']
        else:
    ...
    current_stage_rewards = [get_stage_reward(s, r) for s, r in zip(stage, stage_rewards)]

    id2score = defaultdict(list)
    id2mean = {}
    id2std = {}
    seen_pairs = set()
    scores = [s + 0.5 * cr for s, cr in zip(scores, current_stage_rewards)]
