...
def process_image(image, max_pixels: int = 224 * 224, min_pixels: int = 224 * 224):
...
def adjust_batch(config, data: DataProto, mode="copy") -> DataProto:
    ...
    if mode == "delete":
        ...
        dup_indices = np.random.choice(bs, to_add, replace=True) # TODO
...
def filter_group_data(batch_list : List[Dict],
    ...
    def valid_ratio(last_action_list):
        cnt = 0
        for last_action in last_action_list:
            if 'invalid' in last_action:
                cnt += 1
        if len(last_action_list) == 0:
            return 0
        # print(last_action_list)
        # print(cnt, len(last_action_list), 1 - cnt / len(last_action_list))
        return 1 - cnt / len(last_action_list)

    def valid_ratio_length(length_list):
        cnt = 0
        ...
        if len(length_list) == 0:
            return 0
        return 1 - cnt / len(length_list)

    def valid_ratio_reward(reward_list):
        cnt = 0
    ...
    for i in range(batch_size):
        ...
        # if is_train:
        #     if valid_ratio_length(episode_lengths[group_indices]) >= 0.9 and valid_ratio_reward(group_rewards) >= 0.5:
        #         # If so, keep the entire group, otherwise, remove it
        #         keep_indices = np.concatenate((keep_indices, group_indices))
        # else:
        #     if valid_ratio_length(episode_lengths[group_indices]) >= 1.0:
        #         # If so, keep the entire group, otherwise, remove it
        #         keep_indices = np.concatenate((keep_indices, group_indices))
