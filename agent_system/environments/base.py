        ...
        for i in reversed(range(len(total_batch_list[batch_idx]))):
            ...
            if batch_item['active_masks']:
                ...
                for key in info_keys:
                    if key in ['step_reward', 'step_code', 'invalid_reason']:
                        if key not in success.keys():
                            success[key] = [info[key]]
                        continue
                    if key not in success.keys():
                        success[key] = []
                    success[key].append(info[key])
                # success.update({k: v for k, v in info.items() if k not in success.keys()})
                return
